...
Scheduled syncs skip checksum verification for kept local assets unless `SCHEDULED_CHECKSUM_VERIFICATION_CRON` is set. To run verification only on selected scheduled runs, set it to a cron expression that matches those scheduled run times, for example `0 2 * * 0` for Sunday 02:00. Web UI-triggered syncs and one-off CLI syncs always verify checksums.
